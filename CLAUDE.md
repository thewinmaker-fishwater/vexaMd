...
**커밋(commit)과 푸쉬(push)는 반드시 사용자의 명시적 허가를 받은 후에만 진행한다.**

- 문서 업데이트도 예외 없음
- 작은 수정도 예외 없음
- "커밋해도 될까요?" 또는 "푸쉬해도 될까요?" 확인 필수
- 사용자가 "커밋해", "푸쉬해" 등 명시적으로 요청한 경우에만 진행

### 기타 Git 규칙

1. **테스트 검증 필수**: 테스트 검증이 되지 않은 개발작업은 절대 커밋하지 않음
2. **세션 로그 업데이트**: 커밋 전 SESSION-LOG.md 업데이트
3. **위험한 명령 금지**: `git reset --hard`, `git push --force` 등은 사용자 명시적 요청 시에만
