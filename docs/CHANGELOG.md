...
### 새 기능
- **마크다운 편집기**: 앱 내에서 직접 마크다운 파일 편집 가능
  - **View 모드**: 보기 전용 (기본값)
  - **Edit 모드**: 편집 전용 - 에디터만 전체 화면 표시
  - **Split 모드**: 좌측 에디터, 우측 실시간 미리보기
  - 실시간 미리보기 (300ms 디바운스)
  - `Ctrl+S` 단축키로 저장
  - 변경된 탭에 • 표시 (dirty indicator)
  - 미저장 탭 닫기 시 확인 다이얼로그
  - 콘텐츠 너비 설정에 따른 에디터 너비 연동

### UI/UX
- 에디터 패딩 및 가운데 정렬 (보기 모드와 동일한 레이아웃)
- 에디터 테두리, 둥근 모서리, 그림자 효과
- Split 모드에서 에디터/미리보기 구분선
- 다크 모드 지원

### 관련 파일
- `src/modules/editor/editor.js`: 에디터 모듈 (신규)
- `src/modules/editor/editor.css`: 에디터 스타일 (신규)
- `src/main.js`: 에디터 기능 통합, Ctrl+S 단축키
- `src/components/icons.js`: edit, view, split, save 아이콘
- `src/core/events.js`: 에디터 관련 이벤트 추가
- `src/i18n.js`: 편집 관련 번역 (한/영/일)
- `index.html`: 에디터 패널, 모드 버튼 UI

---

## 2026-01-24 코드 블록 복사 버튼

### 새 기능
- **코드 블록 복사 버튼**: 코드 블록에 마우스를 올리면 복사 버튼 표시
  - 클릭 시 클립보드에 코드 복사
  - 복사 완료 시 체크 아이콘으로 피드백
  - 언어 라벨 표시 (JavaScript, Python 등)
  - 다국어 지원 (한/영/일)

### 관련 파일
- `src/main.js`: 복사 버튼 HTML, 이벤트 핸들러
- `src/modules/viewer/markdown.js`: 복사 버튼 HTML
- `src/modules/viewer/syntax.css`: 복사 버튼 스타일
- `src/i18n.js`: 복사 관련 번역 추가
...
### 새 기능
- **커스텀 테마 저장/관리 (테마 프리셋)**: 여러 개의 커스텀 테마를 저장하고 관리
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-01-24 | 마크다운 편집 기능 추가 |
| 2026-01-24 | 코드 블록 복사 버튼 추가 |
| 2026-01-24 | 코드 문법 하이라이트 추가 |
| 2026-01-24 | PDF 내보내기 기능 추가 |
| 2026-01-24 | 모듈별 CSS 분리 |
| 2026-01-23 | 여러 커스텀 테마 저장 기능 추가 |
| 2026-01-22 | TOC 사이드바, 콜아웃 버그 수정 |
| 2026-01-22 | 싱글 인스턴스 버그 수정 |
