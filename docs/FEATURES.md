...
## 개요
Vexa MD는 Tauri 2.0 기반의 초경량 마크다운 뷰어입니다.
...
- **Ctrl+W**: 현재 탭 닫기
- **Ctrl+Tab**: 다음 탭으로 전환
- **탭 횡스크롤**: 탭이 화면을 초과하면 좌우 방향 버튼(‹/›) 표시, 마우스 휠로 횡스크롤
- **탭별 스크롤 위치**: 탭마다 콘텐츠 스크롤 위치 독립 저장/복원
...
### 즐겨찾기
- **Ctrl+B**: 현재 파일을 즐겨찾기 추가/제거
- 툴바 별표(☆/★) 버튼으로 즐겨찾기 토글
- 즐겨찾기 목록 드롭다운 (파일 열기, X로 제거, 전체 지우기)
- 홈 화면에 즐겨찾기 섹션 표시 (최근 파일 위)
- 최대 20개 저장 (localStorage)
- 탭 전환 시 별표 상태 자동 동기화
...
### 페이지 분량 가이드
- **화면 경계선**: 뷰포트 높이 위치에 점선 가이드 라인 표시
- **넘침 배지**: 페이지별 "적정" (초록) 또는 "넘침 +Npx" (빨강) 상태 표시
- **토글 버튼**: 페이지 네비게이션 영역에서 가이드 on/off
- **자동 갱신**: 창 리사이즈, 페이지 전환 시 자동 재계산
- 페이징 뷰 모드에서만 동작
...
- **F3/Shift+F3**: 다음/이전 결과 (검색창 닫힌 상태에서도 동작)
- 줌 상태에서도 정확한 스크롤 이동
...
- **편집 모드 자동 숨김**: 편집 모드 진입 시 TOC 자동 숨김, 뷰어 모드 복귀 시 자동 복원
...
### 편집 모드
- **도구 > 편집 모드**: 전체 화면 에디터로 마크다운 편집
- **Ctrl+S**: 편집 내용 저장
- TOC 사이드바 자동 숨김

### 분할 모드
- **도구 > 분할 모드**: 좌측 에디터 + 우측 뷰어 동시 표시
- **커서 동기 스크롤**: 에디터에서 클릭/키보드로 커서 이동 시 뷰어가 해당 위치로 자동 스크롤
- 실시간 미리보기
...
### Word Counter (단어 수 카운터) v1.1.0
- 문서의 글자 수, 단어 수를 툴바에 표시
- **CJK 인식 카운팅**: 한국어/중국어/일본어 문자를 개별 단어로 카운팅
- **3가지 표시 모드**: 글자 수만, 단어 수만, 둘 다 표시 (기본: 둘 다)
- **공유 유틸리티**: `text-utils.js`로 상태바와 동일한 카운팅 로직 사용
- 빈 문서에서 '--' 표시
...
- **자동 확인**: 앱 시작 3초 후 백그라운드에서 새 버전 확인
- **수동 확인**: 도움말 > "업데이트 확인" 메뉴
- **업데이트 모달**: 현재 버전 → 새 버전 정보, 다운로드 진행률 바
- **다운로드 후 재시작**: 업데이트 완료 후 "재시작" 버튼으로 즉시 적용
- **서명 검증**: 비대칭 키 기반 업데이트 무결성 검증 (minisign)
- **엔드포인트**: GitHub Releases의 `latest.json` 참조
- **오프라인 대응**: 네트워크 오류 시 조용히 실패 (로그만 출력)
...
- **도구(Tools) 드롭다운**: 인쇄, 프레젠테이션, TOC, 편집 모드, 저장, VMD 내보내기
- **상호 배타적**: 하나 열면 다른 것 자동 닫힘
- **외부 클릭**: 드롭다운 외부 클릭 시 자동 닫힘
...
- **툴바 버튼**: 커서↔손 모드 전환
- **커서 모드**: 텍스트 선택/복사 가능 (기본)
- **손끌기 모드**: 드래그로 콘텐츠 스크롤 이동
...
- **Ctrl+/**: 상태바 표시/숨기기 토글
- **하단 고정**: VS Code 스타일 하단 상태바 (높이 28px)
- **문서 통계**: 글자 수, 단어 수, 예상 읽기 시간 표시
- **파일 정보**: 현재 파일명 표시 (홈탭에서는 앱 이름 표시)
- **줌 비율**: 현재 줌 레벨 표시
- **다국어 지원**: 통계 단위 다국어 표시 (자/chars/文字, 단어/words/語, 분/min/分)
- **CJK 인식 카운팅**: 한국어/중국어/일본어 문자를 정확하게 카운팅
- **실시간 업데이트**: 파일 로드, 탭 전환, 에디터 편집, 줌 변경 시 자동 업데이트
- **테마 연동**: 다크/라이트 테마 자동 대응 (CSS 변수)
- **인쇄/프레젠테이션 제외**: 인쇄 및 프레젠테이션 모드에서 자동 숨김
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-03-06 | 마크다운 편집기(분할 모드 커서 동기화), 페이지 분량 가이드, 검색 개선(F3, 포커스), TOC 편집모드 숨김 |
| 2026-02-20 | 상태바 추가, Word Counter v1.1.0, Ctrl+/ 단축키 |
| 2026-02-14 | 자동 업데이트, 툴바 드롭다운, 커서/손끌기, 제품명 Vexa MD로 통일 |
| 2026-02-02 | 탭 횡스크롤 및 방향 버튼, 탭별 스크롤 위치 저장/복원 |
| 2026-02-01 | 내장 플러그인 8종 추가 |
| 2026-01-31 | 플러그인 시스템 Phase 2 추가 |
