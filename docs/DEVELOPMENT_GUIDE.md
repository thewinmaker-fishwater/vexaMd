...
프로덕션 빌드 시 Terser를 사용하여 프론트엔드 JavaScript 소스를 난독화합니다:

| 옵션 | 설명 | 효과 |
|------|------|------|
| `drop_console` | `console.*` 호출 제거 | 로그 노출 방지 |
| `drop_debugger` | `debugger` 문 제거 | 디버그 포인트 제거 |
| `passes: 2` | 압축 2회 반복 | 더 작은 번들 크기 |
| `toplevel: true` | 최상위 변수명 난독화 | 변수/함수명 보호 |
| `properties.regex: /^_/` | `_`로 시작하는 속성명 난독화 | 내부 속성 보호 |
| `comments: false` | 모든 주석 제거 | 소스 정보 노출 방지 |

> **참고**: Terser는 `devDependencies`에 포함되어 있어야 합니다 (`npm install --save-dev terser`).
> 개발 모드(`npm run tauri dev`)에서는 난독화가 적용되지 않습니다.
...
Vite + Terser를 통한 프론트엔드 소스 보호 (§3.2 참조):
- **변수/함수명 난독화**: 원본 변수명을 짧은 식별자로 변환
- **콘솔/디버거 제거**: 프로덕션에서 디버그 정보 노출 방지
- **주석 제거**: 소스 코드 주석 완전 제거
- **다중 패스 압축**: 2회 반복 압축으로 번들 크기 최소화
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-03-06 | Vite + Terser 소스 난독화 설정 문서화 |
