...
| 상태바 (Status Bar) | 2026-02-20 | status-bar/status-bar.js, status-bar.css, index.html |
| 공유 텍스트 유틸리티 (text-utils.js) | 2026-02-20 | core/text-utils.js |
| Word Counter v1.1.0 (CJK 인식) | 2026-02-20 | plugins/word-counter/index.js |
| 플러그인 설정 UI 스크롤/슬라이더 수정 | 2026-02-20 | plugin-ui.css, plugin-ui.js |
| 뷰 모드/에디터 모드 토글 스타일 개선 | 2026-02-20 | ui.css |
| 저장 버튼 에디터 모드 전용 표시 | 2026-02-20 | editor-manager.js |
...
| # | 기능 | 설명 | 난이도 |
|:-:|------|------|:------:|
| ~~1~~ | ~~상태바 (Status Bar)~~ | ~~하단에 글자 수, 단어 수, 읽기 시간, 파일 경로, 줌 비율 표시~~ | ✅ |
| 2 | 즐겨찾기 (Favorites) | 자주 여는 파일을 즐겨찾기로 고정, 홈 화면에서 빠른 접근 | ⭐ |
| 3 | HTML 단독 내보내기 | CSS 포함된 단독 HTML 파일로 내보내기, 브라우저에서 바로 열기 가능 | ⭐⭐ |
| 4 | 마크다운 테이블 편집기 | GUI로 테이블 행/열 추가/삭제/편집, 에디터 모드 연동 | ⭐⭐⭐ |
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-02-20 | 상태바 완료 반영, Word Counter v1.1.0/text-utils/플러그인 UI 수정 추가 |
