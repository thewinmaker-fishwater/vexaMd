...
export function init({ t, doRenderMarkdown }) {
  ...
  if (editorTextarea) {
    ...
    editorTextarea.addEventListener('click', onEditorCursorMove);
    editorTextarea.addEventListener('keyup', onEditorCursorMove);
  }
}
...
// 분할 모드: 에디터 커서 위치에 따라 뷰어 자동 스크롤
function onEditorCursorMove() {
  if (!mainContainer?.classList.contains('mode-split')) return;
  if (!editorTextarea || !contentEl) return;

  // 에디터 내 커서의 상대 위치 비율 계산
  const cursorPos = editorTextarea.selectionStart;
  const totalLength = editorTextarea.value.length;
  if (totalLength === 0) return;
  const ratio = cursorPos / totalLength;

  // 뷰어의 스크롤 가능 영역에 비율 적용
  const maxScroll = contentEl.scrollHeight - contentEl.clientHeight;
  if (maxScroll <= 0) return;
  contentEl.scrollTo({ top: ratio * maxScroll, behavior: 'smooth' });
}
