...
function updateTocVisibility() {
  // 편집 모드에서는 TOC 표시하지 않음
  const isEditMode = mainContainer?.classList.contains('mode-edit');
  if (tocVisible && currentHeadings.length > 0 && !isEditMode) {
...
export function initToc() {
  ...
  // 에디터 모드 변경 시 TOC 가시성 재평가
  eventBus.on(EVENTS.EDITOR_MODE_CHANGED, () => updateTocVisibility());
}
